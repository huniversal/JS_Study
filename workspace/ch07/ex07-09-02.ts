...
(() => {
  const startDate = new Date("2025-03-27"); // 특정 날짜 넘겨도 된다
  ...
  function getDate(date: Date): string {
    ...
    return `${date.getFullYear()}-${date.getMonth() + 1 < 10 ? "0" + (date.getMonth() + 1) : date.getMonth() + 10}-${date.getDate() < 10 ? "0" + date.getDate() : date.getDate()}`;
  }
})();
